...
def open_main_app(): #INSERIR COMO ARGUMENTO DA FUNÇÃO: username, para exibir o nome de maneira dinâmica
    ...
    def open_window(title):
        new_window = Toplevel(main_app)
        new_window.title(title)
        new_window.geometry("400x300")
        tk.Label(new_window, text=title, font=("Arial", 18)).pack(pady=20)
        tk.Label(new_window, text="Conteúdo da janela de " + title).pack()
    ...
    def criar_quadrados_info(main_frame, estoque):
    ...
    criar_quadrados_info(Frame_App, total_produtos)
